    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if count == 3 {
            ...
            let okAction = UIAlertAction(title: "Sure!", style: .default) { (action) in
                ...
                if let url = URL(string: "http://itunes.apple.com/WebObjects/MZStore.woa/wa/viewContentsUserReviews?id=396356844&pageNumber=0&sortOrdering=2&type=Purple+Software&mt=8") {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toSlaveView" {
            if let index = tableView.indexPathForSelectedRow,
                ...
                if index.section == 0 {
                    destinationVC.updateSlave(volume: volumes[index.row], showHeader: false)
